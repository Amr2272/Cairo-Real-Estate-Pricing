...
CATEGORICAL_FEATURES = [
    'district', 'compound_name', 'finishing_type', 'view_type'
...
def train_and_save_model(data_path, model_path, features_path):
    ...
    ui_features['numeric_features'] = {col: {'min': X[col].min(), 'max': X[col].max()} for col in NUMERIC_FEATURES}
    
    with open(features_path, 'w') as f:
        json.dump(ui_features, f)
    print(f"UI features saved to {features_path}")
